/**
 * Country calling codes and lightweight phone-number validation for checkout.
 *
 * The country/calling-code list is based on the public country telephone data
 * maintained by mukeshsoni/country-telephone-data.
 *
 * Validation is deliberately conservative: it validates the selected calling
 * code, country-specific national-number length, E.164's 15-digit ceiling,
 * and obvious placeholder/repeated-digit values. Cashfree still performs its
 * own gateway validation when the order is created.
 */
...
type RawCountry = readonly [name: string, iso2: string, dialCode: string, format: string];
...
const COUNTRY_MAP = new Map(PHONE_COUNTRIES.map((country) => [country.iso2, country]));

export const DEFAULT_PHONE_COUNTRY = "IN";

export function getPhoneCountry(iso2: string): CountryPhone | undefined {
  return COUNTRY_MAP.get(iso2.toUpperCase());
}

export function normalizeNationalPhone(value: string, country: CountryPhone): string {
  let digits = value.replace(/\D/g, "");
  const dialDigits = country.dialCode.slice(1);

  if (digits.startsWith("00" + dialDigits)) digits = digits.slice(dialDigits.length + 2);
  else if (digits.startsWith(dialDigits) && digits.length > country.maxLength) digits = digits.slice(dialDigits.length);
  else if (digits.startsWith("0") && digits.length > country.maxLength) digits = digits.slice(1);
...
export function validatePhoneNumber(
  iso2: string,
  value: string,
): { valid: true; e164: string; digits: string } | { valid: false; error: string } {
  const country = getPhoneCountry(iso2);
  if (!country) return { valid: false, error: "Select a valid country calling code." };
  ...
  if (e164.replace(/\D/g, "").length > 15) {
    return { valid: false, error: "Phone numbers cannot exceed 15 digits internationally." };
  }
