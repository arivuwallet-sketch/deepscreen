...
The follow-up screenshot request also restores the original homepage top-score selection and table heading, stock table values and score/verdict display, stock analysis panels and summary, underlying merge/derivation behavior, and exchange default score ordering from pre-optimization commit 1a3eaf3. Contact and subscription form presentation returns to the earlier layout; the added checklist subscription box is removed. Insert-only newsletter storage support and subscriber records are retained so the earlier form continues to work without exposing subscriber data. SEO metadata, source-aware FAQ schema, crawlable pagination, canonical redirects, sitemaps, resource pages and the public reference API remain. Discovery text now describes the restored fallback behavior accurately.
