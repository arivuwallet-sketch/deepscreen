-- Defense-in-depth payment integrity:
-- 1) constrain payment-order state and amount,
-- 2) keep payment activation atomic and idempotent inside one database transaction,
-- 3) make the client unable to grant/alter subscription state directly.

ALTER TABLE public.payment_orders
  ADD COLUMN IF NOT EXISTS activated_at TIMESTAMPTZ;

ALTER TABLE public.payment_orders
  DROP CONSTRAINT IF EXISTS payment_orders_tier_check,
...
ALTER TABLE public.payment_orders
  ADD CONSTRAINT payment_orders_tier_check
    CHECK (tier IN ('weekly', 'monthly', 'annual')),
  ADD CONSTRAINT payment_orders_currency_check
    CHECK (currency = 'INR'),
  ADD CONSTRAINT payment_orders_amount_check
    CHECK (amount > 0 AND amount < 100000000),
  ADD CONSTRAINT payment_orders_status_check
    CHECK (status IN ('created', 'paid', 'underpaid', 'failed', 'expired'));
